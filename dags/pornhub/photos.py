...
class Photos(object):
    ...
    def _loadAlbumsPage(self, page_num):
        ...
        r = requests.get(search_url, params=payload, headers=HEADERS, proxies=self.ProxyDictionary, cookies=COOKIES)
        html = r.text
    ...
    def _scrapPhotoFullURL(self, preview_url):
        r = requests.get(preview_url, headers=HEADERS, proxies=self.ProxyDictionary, cookies=COOKIES)
        html = r.text
    ...
    def _scrapAlbumPhotos(self, album_url):
        r = requests.get(album_url, headers=HEADERS, proxies=self.ProxyDictionary, cookies=COOKIES)
        html = r.text
        soup = BeautifulSoup(html, "lxml")
